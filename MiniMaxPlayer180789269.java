...
class MiniMaxPlayer180789269 extends GomokuPlayer {
    ...
    private int maxABValue(Color[][] board, Color maxColor, Color minColor, int alpha, int beta){
        int value = -2;
        BoardAnalysis bd = boardAnalyser(board);
        if (bd.winner != null) {
            return -1;
        ...
        for (Move legalMove : bd.legalMoves){
            ...
            value = Math.max(value,minABValue(cloneBoard, maxColor, minColor, alpha, beta));
            if (value >= beta){
                return value;
            }
    ...
    private int minABValue(Color[][] board, Color maxColor, Color minColor, int alpha, int beta){
        int value = 2;
        BoardAnalysis bd = boardAnalyser(board);
        if (bd.winner != null) {
        ...
        for (Move legalMove : bd.legalMoves){
            ...
            value = Math.min(beta,maxABValue(cloneBoard, maxColor, minColor, alpha, beta));
            if (value <= alpha){
                return value;
            }
